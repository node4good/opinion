...
function view(dir, opts) {
    ...
    // default extname
    var defaultExtension = opts.ext || 'html';
    ...
    return function (view, locals) {
        ...
        // map engine
        var engineName = map[ext] || ext;
        var engine = cons[engineName];

        // resolve
        var viewPath = join(dir, view);

        // cache
        locals.cache = cache;

        debug('render %s %j', viewPath, locals);

        return function (done) {
            debug('render %s with %j', viewPath, opts);
            engine(viewPath, opts, done);
        }
    };
}
